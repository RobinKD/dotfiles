...
#+begin_src elisp
  (defun get-meow-selection ()
    (if (symbol-value 'meow--selection)
	(let* ((meow-selected (cdr (symbol-value 'meow--selection)))
	       (argone (pop meow-selected))
	       (argtwo (pop meow-selected)))
	  (buffer-substring argone argtwo))
      )
    )
			       ...
			       :children (("Meeting" :keys "m"
					  ...
					  ("Regular advancement meeting" :keys "r"
					   ...
					   :template ("* Regular advancement meeting"
						      "SCHEDULED: %^{Date:}T"
						      "https://orga2.webex.com/meet/Username2"))
					  ))
			      ("Formations" :keys "f"
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/snippets/org-mode/beamertheme_short-orga1 :mkdirp yes
# -*- mode: snippet -*-
# name: themeshort-orga1
# key: themeshort-orga1
# uuid: themeshort-orga1
# expand-env: ((yas/indent-line 'fixed))
# --
...
topdir = home+'/Templates/beamer_templates/short-orga1/'
save_dir = './'
def copy_template():
    to_copy = []
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/snippets/org-mode/beamertheme_simpleshort-orga1 :mkdirp yes
# -*- mode: snippet -*-
# name: themesimpleshort-orga1
# key: themesimpleshort-orga1
# uuid: themesimpleshort-orga1
# expand-env: ((yas/indent-line 'fixed))
# --
#+author: Name1
#+date: $1
...
#+latex_class_options: [presentation,10pt]
#+columns: %45ITEM %10BEAMER_env(Env) %10BEAMER_act(Act) %4BEAMER_col(Col) %8BEAMER_opt(Opt)
#+beamer_theme: Simpleshort-orga1
#+beamer_header: \usepackage[utf8]{inputenc}
#+beamer_header: \usepackage{hyperref}
#+beamer_header: \usepackage{graphics}
#+beamer_header: \usepackage[export]{adjustbox}
...
# The top argument for walk
home = os.path.expanduser("~")
topdir = home+'/Templates/beamer_templates/Simpleshort-orga1/'
save_dir = './'
def copy_template():
    to_copy = []
    for path, dirnames, files in os.walk(topdir):
  ...
  (use-package mu4e
    ...
    (setq mu4e-bookmarks
	  '((:name "Unread messages"
		   :query "flag:unread AND NOT maildir:/hotmail/Deleted AND NOT maildir:/short-orga2/Trash AND NOT maildir:/en/Corbeille"
		   :key ?u)
	    (:name "Today's messages"
		   :query "date:today..now AND NOT maildir:/hotmail/Deleted AND NOT maildir:/short-orga2/Trash AND NOT maildir:/short-orga2/Sent AND NOT maildir:/en/Envois AND NOT maildir:/en/Corbeille AND NOT maildir:/hotmail/Sent"
		   :key ?t)
	    (:name "Last 7 days"
		   :query "date:7d..now AND NOT maildir:/hotmail/Deleted AND NOT maildir:/short-orga2/Trash AND NOT maildir:/short-orga2/Sent AND NOT maildir:/en/Envois AND NOT maildir:/en/Corbeille AND NOT maildir:/hotmail/Sent"
		   :key ?w)
...
#+begin_src elisp
  (setq mu4e-contexts
	(list
	 (make-mu4e-context
	  :name "short-orga2"
	  :enter-func (lambda () (mu4e-message "Entering context short-orga2"))
	  :leave-func (lambda () (mu4e-message "Leaving context short-orga2"))
	  :match-func (lambda (msg)
			(when msg
			  (string-match-p "^/short-orga2" (mu4e-message-field msg :maildir))))
	  :vars '((user-mail-address . "example2@email.com")
		  (user-full-name . "Name1")
		  (mu4e-sent-folder . "/short-orga2/Sent")
		  (mu4e-drafts-folder . "/short-orga2/Drafts")
		  (mu4e-trash-folder . "/short-orga2/Trash")
		  (org-msg-signature . "\n--\nName1\nDoctorant\nOrga1\nOrga2 / Orga3")
		  (mu4e-compose-format-flowed . t)
		  (smtpmail-queue-dir . "~/.mail/short-orga2/queue/cur")
		  (smtpmail-smtp-user . "example2@email.com")
		  (smtpmail-default-smtp-server . "z.short-orga2.fr")
		  (smtpmail-smtp-server . "z.short-orga2.fr")
		  (smtpmail-smtp-service . 587)
		  (smtpmail-debug-info . t)
		  (mu4e-maildir-shortcuts . ( ("/short-orga2/INBOX"     . ?i)
					      ("/short-orga2/Sent"      . ?s)
					      ("/short-orga2/Trash"     . ?t)
					      ("/short-orga2/Drafts"    . ?d)))))
#+end_src
*** Orga3 context
#+begin_src elisp
  (make-mu4e-context
   :name "Orga3"
   :enter-func (lambda () (mu4e-message "Entering context Orga3"))
   :leave-func (lambda () (mu4e-message "Leaving context Orga3"))
   :match-func (lambda (msg)
		 (when msg
   ...
   :vars '((user-mail-address . "example3@email.com")
	   ...
	   (smtpmail-default-smtp-server . "smtp.orga3.fr")
	   (smtpmail-smtp-server . "smtp.orga3.fr")
	   (smtpmail-smtp-service . 587)
	   (smtpmail-debug-info . t)
...
#+begin_src elisp
  (make-mu4e-context
   :name "orga4"
   :enter-func (lambda () (mu4e-message "Entering context orga4"))
   :leave-func (lambda () (mu4e-message "Leaving context orga4"))
   :match-func (lambda (msg)
		 (when msg
		   (string-match-p "^/orga4" (mu4e-message-field msg :maildir))))
   :vars '((user-mail-address . "Username1@orga4.net")
	   (user-full-name . "Name2")
	   (mu4e-sent-folder . "/orga4/Sent")
	   (mu4e-drafts-folder . "/orga4/drafts")
	   (mu4e-trash-folder . "/orga4/Trash")
	   (mu4e-compose-signature . "Name2")
	   (org-msg-signature . "\n--\nName2")
	   (mu4e-compose-format-flowed . t)
	   (smtpmail-queue-dir . "~/.mail/orga4/queue/cur")
	   (smtpmail-smtp-user . "Username1")
	   (smtpmail-default-smtp-server . "mail.orga4.net")
	   (smtpmail-smtp-server . "mail.orga4.net")
	   (smtpmail-smtp-service . 587)
	   (smtpmail-debug-info . t)
	   (smtpmail-debug-verbose . t)
	   (mu4e-maildir-shortcuts . ( ("/orga4/INBOX"      . ?i)
				       ("/orga4/Sent"     . ?s)
				       ("/orga4/Trash"  . ?t)
				       ("/orga4/Spam"     . ?p)
				       ("/orga4/GameGmails" . ?g)
...
IMAPAccount short-orga2
Host z.short-orga2.fr
User example2@email.com
PassCmd "gpg2 -q --for-your-eyes-only --no-tty -d ~/.password-store/Mail/z.short-orga2.fr.gpg"
AuthMechs LOGIN
SSLType IMAPS
SSLVersions TLSv1.2
CertificateFile /etc/ssl/certs/ca-certificates.crt

IMAPStore short-orga2-remote
Account short-orga2

MaildirStore short-orga2-local
Path ~/.mail/short-orga2/
Inbox ~/.mail/short-orga2/INBOX
SubFolders Verbatim

Channel short-orga2
Far :short-orga2-remote:
Near :short-orga2-local:
Patterns "*"
SyncState *
#+end_src
...
Host imap.orga3.fr
User Username2
PassCmd "gpg2 -q --for-your-eyes-only --no-tty -d ~/.password-store/Work/orga3.fr.gpg"
AuthMechs LOGIN
SSLType STARTTLS
SSLVersions TLSv1.2
CertificateFile /etc/ssl/certs/ca-certificates.crt
...
*** Orga4 with game gmails forwarding account
#+begin_src bash
# ==========================================================================================

IMAPAccount orga4
Host mail.orga4.net
User Username1@orga4.net
PassCmd "gpg2 -q --for-your-eyes-only --no-tty -d ~/.password-store/Mail/orga4.net.gpg"
AuthMechs LOGIN
SSLType IMAPS
SSLVersions TLSv1.3
CertificateFile /etc/ssl/certs/ca-certificates.crt

IMAPStore orga4-remote
Account orga4

MaildirStore orga4-local
Path ~/.mail/orga4/
Inbox ~/.mail/orga4/INBOX
SubFolders Verbatim
